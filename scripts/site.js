Y.use('node', 'squarespace-ui-base', function() {
  ...
  window.Site = Singleton.create({
    ...
    initialize: function() {
      ...
      var node = document.getElementById('parallax-images');


      // Video
      var videos = {
        webm: 'marketing/Background-Footage-v3.webm',
        ogg: 'marketing/Background-Footage-v3.ogv',
        mp4: 'marketing/Background-Footage-v3.mp4'
      };

      var cdn = '//d2zz4thflisoea.cloudfront.net/';

      if (!this.isMobile) {
        this.videoEnabled = true;
        this.video = Y.Node.create('<div id="video-wrap"/>');
        var vv = Y.Node
          .create('<video id="video-background" preload="auto" autoplay loop muted volume="0"/>')
          .appendTo(this.video);
        Y.Object.each(videos, function(filename, type) {
          Y.Node
            .create('<source src="'+ cdn + filename+'" type="video/'+ type +'"/>')
            .appendTo(vv)
        });
        Y.one('div#parallax-images').insert(this.video, 'before');
        Y.one('#video-wrap').setStyle('height', this.firstImageHeight - Y.one('#header').height());
        Y.one('.image-container').setStyle('display', 'none');
      }
    },
    ...
    syncUI: function() {
      ...
      if (Y.one('body.collection-type-index')) {
        ...
        if (this.videoEnabled && Y.one('#video-wrap')) {
          Y.one('#video-wrap').setStyle('height', this.firstImageHeight - Y.one('#header').height());
        }
